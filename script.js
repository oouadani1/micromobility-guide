...
function announceResultsStatus(totalRecommendations) {
  ...
  const optionCountText =
    totalRecommendations === 1
      ? (isSpanishLocale() ? "Una opción de micromovilidad está disponible." : "One micromobility option is available.")
      : (isSpanishLocale()
          ? `${totalRecommendations} opciones de micromovilidad están disponibles.`
          : `${totalRecommendations} micromobility options are available.`);
  const statusText = isSpanishLocale()
    ? `Resultados actualizados. ${optionCountText}`
  ...
  window.requestAnimationFrame(() => {
    window.requestAnimationFrame(() => {
      if (document.getElementById("results-status") === liveRegionEl) {
        liveRegionEl.textContent = statusText;
      }
    });
  });
}
...
function renderCurrentRecommendationPage() {
  ...
  result.innerHTML = `
    <section id="results" aria-labelledby="results-heading" tabindex="-1">
      <div id="results-status" class="sr-only" aria-live="polite" aria-atomic="true"></div>
      ...
      <div class="results-toolbar">
        <div class="results-pager">
          ...
          <button
            ...
            ${showNext ? "" : "hidden aria-hidden=\"true\""}
          >
            <span aria-hidden="true">&#8250;</span>
          </button>
        </div>
